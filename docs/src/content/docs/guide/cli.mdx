...
| `--path` | `/` with domain, `/<name>` without domain | Mount the app under a URL path. |
...
| `--build-command` | `npm run build` | Build command executed after dependency installation. Alias: `--build-cmd`. |
| `--start-command` | `npm run start` | Start command written to the systemd service. Alias: `--start-cmd`. |
| `--force` | `false` | Re-clone even if HEAD did not change. |
...
| `--path` | URL path to mount the app under. |
