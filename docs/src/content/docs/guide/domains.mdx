...
You can also mount several apps under one domain by giving each app a path:
...
APIs usually work well behind a path prefix. Frontend apps may need their framework base path set to the same value, for example `/admin`.
