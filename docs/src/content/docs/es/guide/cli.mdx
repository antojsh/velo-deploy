...
| `--path` | `/` con dominio, `/<name>` sin dominio | Monta la app bajo un path de URL. |
...
| `--build-command` | `npm run build` | Comando de build ejecutado despues de instalar dependencias. Alias: `--build-cmd`. |
| `--start-command` | `npm run start` | Comando de start escrito en el servicio systemd. Alias: `--start-cmd`. |
| `--force` | `false` | Re-clona aunque el HEAD no haya cambiado. |
...
velo-deploy deploy https://github.com/user/api --build-command "pnpm build" --start-command "pnpm start"
...
| `--path` | Path de URL donde montar la app. |
